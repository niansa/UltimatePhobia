...
System_String_o *ToCsString(std::string_view str) {
    if (str == "<null>")
        return nullptr;
    return Il2Cpp::System::String::CreateString(
        nullptr,
        const_cast<signed char *>(
            reinterpret_cast<const signed char *>(str.data())),
        0, str.size());
}
...
std::string ToCppString(System_String_o *str) {
    ...
    return utf8Encode({reinterpret_cast<wchar_t *>(&str->fields.m_firstChar),
                       static_cast<size_t>(str->fields.m_stringLength)});
}
...
std::wstring_view ToCppWideString(System_String_o *str) {
    ...
    return {reinterpret_cast<wchar_t *>(&str->fields.m_firstChar),
            static_cast<size_t>(str->fields.m_stringLength)};
}
