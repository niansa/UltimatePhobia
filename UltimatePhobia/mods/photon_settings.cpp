...
static bool photonNetwork$$ConnectUsingSettingsFnc(
    Photon_Realtime_AppSettings_o *appSettings, bool startInOfflineMode,
    ...
    return self->photonNetwork$$ConnectUsingSettingsHook
        .getFunction<decltype(photonNetwork$$ConnectUsingSettingsFnc)>()(
            appSettings, startInOfflineMode, method);
}

PhotonSettings::PhotonSettings()
    : photonNetwork$$ConnectUsingSettingsHook(GameHook::safeCreateOrPanic(
          photonSettingsInfo,
          Il2Cpp::Photon::Pun::PhotonNetwork::ConnectUsingSettings_getPtr<
              Photon_Realtime_AppSettings_o *, bool>(),
...
#define OVERRIDER(field)                                                       \
    ...
    Checkbox("Override", &settings.override_##field);                          \
    PopID();                                                                   \
    SameLine();                                                                \
    BeginDisabled(!settings.override_##field);
#define CONVBOOLFIELD(field)                                                   \
    OVERRIDER(field) Checkbox(#field, &settings.field);                        \
    EndDisabled()
#define CONVINTFIELD(field)                                                    \
    OVERRIDER(field) InputInt(#field, &settings.field);                        \
    EndDisabled()
#define CONVSTRFIELD(field)                                                    \
    OVERRIDER(field)                                                           \
    InputText(#field, settings.field, sizeof(settings.field));                 \
    EndDisabled()
    CONVLIST
#undef OVERRIDER
#undef CONVBOOLFIELD
...
void PhotonSettings::fromIl2CppClass(
    const Photon_Realtime_AppSettings_Fields& o) {
...
#define CONVSTRFIELD(field)                                                    \
    OVERRIDER(field) ToCString(o.field, settings.field, sizeof(settings.field))
    CONVLIST
#undef OVERRIDER
#undef CONVBOOLFIELD
#undef CONVINTFIELD
