...
extern "C" void *tracerModHook(void *a, void *b, void *c, void *d, void *e,
    ...
    tracerInfo.get<Tracer>()->log(
        fmt::format("{}{}<{}>(...)\n", std::string(depth, '>'), method.name,
                    GameHook::getTrampolineCaller()));

    GameHookRelease GHR(hook);
    ++depth;
    const auto fres = reinterpret_cast<decltype(tracerModHook) *>(
        GameHook::getTrampolineCaller())(a, b, c, d, e, f, g, h);
    --depth;
    return fres;
}
...
void Tracer::uiUpdate() {
    ...
    std::string_view searchString{searchStringBuffer,
                                  strlen(searchStringBuffer)};
    ...
    if (searchString.size() > 3) {
        ...
        for (const Dynamic::Method& method :
             Dynamic::searchMethods(searchString)) {
...
void Tracer::HookButton(std::string_view signature, bool isDefinitelyHooked) {
    ...
    if (ImGui::Button(
            (fmt::format("{} {}", hooked ? '-' : '+', signature)).c_str())) {
        const auto method = Dynamic::getMethod(signature);
        if (!hooked) {
            auto hook = GameHook::safeCreate(
                method.getFullAddress(),
                reinterpret_cast<void *>(hookTrampoline_tracerModHook), true);
            if (!hook.has_value())
                return;
            hooks.emplace(signature,
                          std::make_unique<GameHook>(std::move(*hook)));
        } else {
...
ModInfo tracerInfo{"Tracer", false, []() { return std::make_unique<Tracer>(); },
