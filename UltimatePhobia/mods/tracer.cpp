...
extern "C" void *tracerModHook(void *a, void *b, void *c, void *d, void *e, void *f, void *g, void *h) {
    static int depth = 0;
    if (depth < 0)
        depth = 0;
...
void Tracer::HookButton(std::string_view signature, bool isDefinitelyHooked) {
    ...
    if (ImGui::Button((std::format("{} {}", hooked ? '-' : '+', signature)).c_str())) {
        ...
        if (!hooked) {
            ...
            if (!hook.has_value())
                return;
            hooks.emplace(signature, std::make_unique<GameHook>(std::move(*hook)));
        } else {
...
ModInfo tracerInfo{"Tracer", false, []() { return std::make_unique<Tracer>(); }, []() { tracerInfo.hidden = !Dynamic::isLoaded(); }};
