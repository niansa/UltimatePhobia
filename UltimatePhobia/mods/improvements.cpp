...
static void painKillers$$UseFnc(PainKillers_o *__this, const MethodInfo *method) {
    ...
    if (!hook) {
        g.logger->error("Improved pain killer use function called but hook not set???");
        return;
    }
...
void Improvements::uiUpdate() {
    ...
    using namespace Il2Cpp;
    if (hookToggle("Better sanity pills", hooks, betterPills, PainKillers::NetworkedUse_getPtr(), reinterpret_cast<void *>(painKillers$$NetworkedUseFnc)) && betterPills)
