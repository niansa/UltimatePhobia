...
DLLFFI::DLLFFI(const std::filesystem::path& modPath)
    ...
    auto initImports = reinterpret_cast<decltype(initImportsDummy) *>(
        GetProcAddress(chandle, "initImports"));
    if (!initImports)
...
DLLFFI::~DLLFFI() { FreeLibrary(chandle); }

void DLLFFI::simpleCall(const char *name) {
    void simpleCallDummy();
    auto func = reinterpret_cast<decltype(simpleCallDummy) *>(
        GetProcAddress(chandle, name));
    g.logger->flush();
    if (!func)
        g.logger->error(
            "Failed to call DLL function {} in {} as it wasn't found", name,
            modPathStr);
    else
        func();
    g.logger->flush();
