...
Playback::Playback(FFIInterface::ObjectHandle audioSource,
                   IPLAudioBuffer audioBuffer, uint64_t delay,
                   float volumeScale, bool isOneShot)
    : audioSource(audioSource), audioBuffer(audioBuffer), delay(delay),
      volumeScale(volumeScale), isOneShot(isOneShot) {
    audioSourceGc = FFI gcCreateHandle(audioSource, true);

    {
        IPLBinauralEffectSettings effectSettings{.hrtf = env->hrtf};
        iplBinauralEffectCreate(GlobalState::phononCtx, &env->audioSettings,
                                &effectSettings, &binauralEffect);
    }

    {
        ...
        iplDirectEffectCreate(
            GlobalState::phononCtx,
            const_cast<IPLAudioSettings *>(&env->audioSettings),
...
void dataCallback(ma_device *pDevice, float *pOutput, const float *pInput,
    ...
    IPLerror status = iplAudioBufferAllocate(GlobalState::phononCtx,
                                             FixedSettings::outputChannels,
                                             sampleCount, &frameBuffer);
...

    {
        ...
        for (auto& playback : playbackQueue) {
            ...
            IPLerror status = iplAudioBufferAllocate(
                GlobalState::phononCtx, playback.audioBuffer.numChannels,
                sampleCount, &inputBuffer);
            if (status != IPL_STATUS_SUCCESS)
                ...
                for (unsigned sample = 0; sample != inputBuffer.numSamples;
                    ...
                    if (sample < availableSamples)
                        inputBuffer.data[channel][sample] =
                            ...
                            (playback.volume * playback.volumeScale);
                    else
            ...
            // Apply binaural effect
            {
                ...
                effectParams.direction = iplCalculateRelativeDirection(
                    GlobalState::phononCtx, playback.worldPosition,
                    GlobalState::playerPos, GlobalState::playerAhead,
                    GlobalState::playerUp);
                effectParams.direction.x = -effectParams.direction.x;
                iplBinauralEffectApply(playback.binauralEffect, &effectParams,
                                       &inputBuffer,
            ...
            // Apply direct effect without simulation
            {
                ...
                IPLDirectEffectParams directEffectParams{
                    ...
                    .distanceAttenuation = iplDistanceAttenuationCalculate(
                        GlobalState::phononCtx, playback.worldPosition,
                        GlobalState::playerPos, &distanceAttenuationModel)};
                iplAirAbsorptionCalculate(
                    GlobalState::phononCtx, playback.worldPosition,
                    GlobalState::playerPos, &airAbsorptionModel,
                    directEffectParams.airAbsorption);
                iplDirectEffectApply(playback.directEffect, &directEffectParams,
                                     &env->bufferPool.GetCurrentBuffer(),
                                     &env->bufferPool.GetNextBuffer());
            }

            // Mix into frame buffer
            iplAudioBufferMix(GlobalState::phononCtx,
                              &env->bufferPool.GetNextBuffer(), &frameBuffer);
